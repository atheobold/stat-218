...
\key{Assuming we have 35 students in the class: $35 \cdot 0.5 = 17.5$.}
...
\key{$\frac{(34-17.5)^2}{17.5} = 15.56$ and $\frac{(1-17.5)^2}{17.5} = 15.56$}
...
\key{Heads = ``Correct Guess", Tails = ``Incorrect Guess"}
...
\key{Our observed statistic falls far to the right of the chi-square distribution, with 0 (out of 1000) simulations as large or larger than it. For this reason, our p-value is estimated to be about 0 or (< 0.001).}

\vspace{0.1in}

**11. Based on your p-value, what would you conclude for your null and alternative hypotheses from #2 and #3?**

\key{Since our p-value is small (< 0.001) and it is less 0.05, we would reject the null hypthesis that there is an equal chance of correctly and incorrectly guessing. We would conclude that it seems more likely that the probability of correctly identifying ``Kiki" is not equal to 50\%.}
...
\key{The observations are independent because each student's response does not tell us information about another student's response. Therefore, independence is not violated. We \emph{do} have a ``large enough" sample size, as the expected counts are both larger than 5! Therefore, we can use the $\chi^2$ distribution to approximate the true sampling distribution. }
...
\key{With a p-value of 0.0067 we would reject the null hypothesis that there is an equal probability of correctly and incorrectly guessing, as our p-value is smaller than our $\alpha$ of 0.05.}

\vspace{0.1in}

**15. Did you reach similar conclusions using simulation-based methods? Why do you believe that is the case?**

\key{Yes, our conclusion with the simulation based method is similar to that of the theory based method, as both methods led us to reject the null hypothesis. These results should be simular because we did not violate the ``large sample" condition (expected counts > 5). If we had violated this condition, using a $\chi^2$ distribution would have incorrectly estimated our p-value and the two methods would have produced different results.}
...
|----------------|-------|--------------------------|
| Inner Burn     | 520   | \key{520 / 3000 = 0.1733}|
| Inner Edge     | 210   | 0.070                    |
| Outer Edge     | 240   | \key{240 / 3000 = 0.08}  |
...
\key{We should use a $\chi^2$ with 3 degrees of freedom (4 regions - 1).}
...
\key{The p-value of less than 0.0001 is less than the alpha threshold cutoff of 0.05; therefore we reject the null hypothesis that the deer show no preference for any specific region and the frequencies of dear are proportional to the size of the region. We would conclude that there is evidence the deer have preference for at least one parcel more than what is expected for its size. }
