...
14. Using the formula above, calculate how far "off" each of the cells in our observed table is from what was expected under the null hypothesis. *Hint:* You
should be adding up **eight** numbers for your $X^2$ statistic!
...
For the Chi-Squared test of independence we've been discussing, it can be mathematically shown that the distribution of $X^2$ statistics follows a $\chi^2$ distribution with $(r - 1) \times (c - 1)$ degrees of freedom, where $r$ is the number of rows and $c$ is the number of columns in the table. Keep in mind the 
total row and column don't count!
...
-   We have a "large enough" sample size
    -   This is checked by verifying there are **at least 5** 
    [expected counts]{.underline} in each cell
...
We will use the `pchisq()` function (which you saw last week). This function 
has three inputs:
...
using cards. Keep in mind, we are **assuming the null hypothesis is true**,
which suggests there is no relationship between the location of someone's injury
