...
\noindent \textbf{Q8} [3 pts] When you change from a 90% to a 95% confidence
...
\textbf{(d)} [2 pts] A scatterplot displaying the number of girls born over time
...
\begin{itemize}
\item[(i)] If there is no linear relationship between the total calories and the
...
\item[(ii)] If there is a linear relationship between the total calories and the
...
\item[(iii)] The probability of seeing a sample slope between the total calories and
the saturated fat of a fast food item of 0.017 or more extreme is less than 0.1%.

\item[(iv)]  The probability that there is no linear relationship between the total
...
\noindent
\textbf{(d)} [2 pts] The Statistician carried out a simulation with 1000 
...
\noindent
\textbf{(e)} [2 pts] The table below presents percentiles for the bootstrap 
...
\noindent
\textbf{(f)} [5 pts] Interpret the 99% confidence interval found in part (f)
...
**Please Note**: Genotypes Tt and tT can be considered equivalent.  
...
\noindent \textbf{(h)} [2 pts] The p-value associated with your $X^2$
...
\noindent \textbf{(b)} [2 pts] Which types of sampling bias may be present in
