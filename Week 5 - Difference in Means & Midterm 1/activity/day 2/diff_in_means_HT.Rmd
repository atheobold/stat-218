...
3. Which group (El Ni\latexcode{\~{n}}o or La Ni\latexcode{\~{n}}a) has the
...
10. Calculate the observed statistic of interest (difference in means).  Use El Ni\latexcode{\~{n}}o minus La Ni\latexcode{\~{n}}a as the order of subtraction.
What is the appropriate notation for this statistic?


__Step 2: Conduct a Hypothesis test__

Remember that the null distribution is created based on the assumption the null hypothesis is true.  In this study, the null hypothesis states that **there is
...
snow %>% 
  ...
  hypothesise(null = __________) %>% 
  
  generate(reps = ______, type = __________) %>% 
...
- make two new groups of sizes $n_1$ and $n_2$
