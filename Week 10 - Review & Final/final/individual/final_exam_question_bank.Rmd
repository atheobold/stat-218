...
Mendelian inheritance refers to the inheritance of traits controlled by a single gene with two alleles, one of which may be completely dominant to the other.
You can use a Punnett square to easily determine the expected ratios of 
...
\noindent \textbf{(b)} If the Mendelian inheritance is true, what
...
\noindent \textbf{(c)} Actually, our table could be a bit simpler. Both the TT
and Tt genotypes will present as "tall" plants, whereas tt genotypes will
present as "short" plants.*
...
\noindent \textbf{(c)} After you cross your plants, you measure the
...
\noindent \textbf{(d)} Calculate how far "off" was your observed number of tall
plants from what you expected if $H_0$ was true. Use these values to report the 
$X^2$ statistic for your experiment. 
...
\noindent \textbf{(e)} The p-value associated with your $X^2$ statistic is 
`r pchisq(0.3319, df = 1, lower.tail = FALSE)`. Your Biology textbook suggests 
you interpret this value as:
...
pchisq(_____________, 
       df = ______________, 
       lower.tail = ___________)
```
...
"Hard" for "attractive" professors compared to "unattractive" professors.
...
They used computer simulation to obtain the null distribution below. 

Fill in the steps necessary to obtain out **one** simulation on the null 
distribution. 
...
\textbf{For each of the following, select the single most appropriate analysis for the situation described. You may use an analysis for more than one situation.} (2 pts each)

\begin{tcolorbox}
CTI - Chi Square Test of Independence \hspace{0.25in} OWA - One Way ANOVA \\
SLR - Simple Linear Regression \hspace{0.25in} MLR - Multiple Linear Regression  \\
TWA - Two Way ANOVA \hspace{0.25in} CTH -  Chi Square Test of Homogeneity \\
\end{tcolorbox}


\item \underline{\hspace{1in}} Researchers are interested in investigating how the number of visitors to Yellowstone National Park in a year impacts the local economy in Bozeman. To do this they count the number of yearly visitors to Yellowstone and measure the dollars spent by tourists in Bozeman for the year.

 
\item  \underline{\hspace{1in}} A study of honeybees looked at whether the proportions of different honeybee species varies by state.  Ten states were used in the study, and 100 honeybees were randomly sampled in each state, and 7 different species were seen in the data set. 

\item \underline{\hspace{1in}} An attorney in Boston observes that some judges seem to select juries that contain few women. She collects data on 20 randomly selected juries from each of 10 judges, and the number of women on each jury for each judge. 

\item \underline{\hspace{1in}} Researchers are interested in whether the linear relationship between mean daytime temperature and number of tomatoes produced differs among three tomato varieties.

\item \underline{\hspace{1in}} Researchers are interested in what gets users more Tinder messages. They collect data on Tinder users' gender (Male, Female, A-gender), race (Black, Caucasian, Asian, Latino), and the number of messages they get in one day for a random sample of 1000 Tinder users.  \\
