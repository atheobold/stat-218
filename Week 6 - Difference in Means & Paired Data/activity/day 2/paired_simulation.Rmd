...
pictured in the figures below, which seem to indicate, perhaps unsurprisingly,
that fewer people driving and factories being shut down have reduced air
pollutants. 
...
ggplot(data = air, 
  ...
  labs(x = "Difference in air quality score (Current - 5 Year Median)") 
```

Summary statistics for current AQI scores, median AQI scores from 2015--2019, and the differences in AQI scores.
...
air %>%
  ...
  kable(col.names = c("", "Mean", "Standard Deviation")
        ) %>% 
  kable_styling()
...
Take for example Albuquerque, New Mexico. In June 2020 the current AQI score was
...
likely to see an AQI score of 30 in June of 2020. 

10. How can we use a coin to decide which values for each city are randomly
assigned to the "Current" (June 2020) time period? 
...
13. Find the observed difference on the distribution. 
...
17. What is the parameter in this study?
...
19. Use the table below to find a 99% confidence interval for $\mu_{diff}$. 
...
mean or mean difference, label $n$ cards with the original differences.
